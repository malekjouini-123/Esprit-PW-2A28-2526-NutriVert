...
$users = $userModel->getAllWithFaceStatus();
$faceCount = count(array_filter($users, function($u) {
    return !empty($u['has_face']);
}));
?>
<!DOCTYPE html>
<html lang="fr">
...
<div class="sidebar">
    ...
    <div class="menu-item" data-section="faces"><i class="fas fa-user-shield"></i><span> Face ID</span></div>
    <a class="menu-item" href="index.php?action=admin-chats"><i class="fas fa-comments"></i><span> Chats IA</span></a>
...
<div class="main">
    ...
    <div id="users-section" class="section">
        <div class="form-card">
            ...
            <!-- LIGNE DU HAUT : formulaire gauche | tableau droit -->
                ...
                <div style="flex:0 0 320px; min-width:260px;">
                    ...
                    <div id="editUserForm" style="display:none;">
                        <form id="editUserFormElement" method="post">
                            ...
                            <div class="form-group"><label>Nom</label><input type="text" name="nom" id="editNom"></div>
                            <div class="form-group"><label>Prénom</label><input type="text" name="prenom" id="editPrenom"></div>
                            <div class="form-group"><label>Email</label><input type="text" name="email" id="editEmail"></div>
                            <div class="form-group"><label>Mot de passe (laisser vide pour ne pas changer)</label><input type="password" name="password" id="editPassword"></div>
                            <div class="form-group"><label>Rôle</label><select name="role" id="editRole"><option value="client">Client</option><option value="admin">Admin</option></select></div>
                            <button type="submit" name="edit_user" class="btn-primary"><i class="fas fa-save"></i> Modifier</button>
                            <button type="button" onclick="hideEditForm()" class="btn-outline"><i class="fas fa-times"></i> Annuler</button>
                        </form>
                    </div>
                </div>
                ...
                <div style="flex:1; min-width:300px;">
                    ...
                    <div style="display:flex; align-items:center; gap:0.8rem; margin-bottom:1rem;">
                        <div style="position:relative; flex:1;">
                            <i class="fas fa-search" style="position:absolute; left:1rem; top:50%; transform:translateY(-50%); color:#4caf50; font-size:0.95rem;"></i>
                            <input type="text" id="searchUser" placeholder="Rechercher par nom, prénom ou email..." oninput="filterUsers()" style="padding-left:2.5rem; border-radius:2rem; border:1.5px solid #4caf50; background:#f9fff7; width:100%;">
                        </div>
                        <button onclick="clearSearch()" class="btn-outline btn-sm"><i class="fas fa-times"></i> Effacer</button>
                    </div>
                    <div id="noResultMsg" style="display:none; color:#888; text-align:center; padding:1rem; font-style:italic;">
                        <i class="fas fa-search" style="margin-right:0.4rem;"></i> Aucun utilisateur trouvé.
                    </div>
                    <!-- Tableau -->
                    <div class="table-container">
                        <table>
                            <thead><tr><th>Nom</th><th>Prénom</th><th>Email</th><th>Rôle</th><th>Face ID</th><th>Actions</th></tr></thead>
                            <tbody id="usersTableBody">
                                <?php foreach ($users as $u): ?>
                                <tr data-nom="<?= htmlspecialchars($u['nom']) ?>" data-id="<?= $u['id_utilisateur'] ?>">
                                    ...
                                    <td>
                                        ...
                                        <?php else: ?>
                                            <span class="badge" style="background:#f5d5d5; color:#8a1c1c;">Absent</span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <button onclick="editUser(<?= $u['id_utilisateur'] ?>, '<?= htmlspecialchars($u['nom']) ?>', '<?= htmlspecialchars($u['prenom']) ?>', '<?= htmlspecialchars($u['email']) ?>', '<?= $u['role'] ?>')" class="btn-outline btn-sm" style="margin-right:0.5rem;"><i class="fas fa-edit"></i> Modifier</button>
                                        <a href="index.php?action=admin&delete_user=<?= $u['id_utilisateur'] ?>" onclick="return confirm('Supprimer cet utilisateur ?')" class="btn-outline btn-sm"><i class="fas fa-trash"></i> Supprimer</a>
                                        <a href="index.php?action=admin-view-face&user_id=<?= $u['id_utilisateur'] ?>" class="btn-outline btn-sm" style="margin-top:0.5rem;"><i class="fas fa-eye"></i> Face ID</a>
                                        <br><a href="index.php?action=logout" class="btn-outline btn-sm" style="margin-top:0.5rem;"><i class="fas fa-sign-out-alt"></i> Déconnexion</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                    ...
                    <!-- Bouton de tri -->
                    <div style="display:flex; justify-content:flex-end; margin-top:0.8rem; gap:0.5rem; align-items:center;">
                        <i class="fas fa-sort" style="color:#4caf50;"></i>
                        <select onchange="sortUsers(this.value)" style="width:auto; padding:0.4rem 1rem; border-radius:2rem; border:1.5px solid #4caf50; background:#f9fff7; color:#2e7d32; font-weight:600; cursor:pointer; font-size:0.85rem;">
    ...
    <div id="faces-section" class="section">
        <div class="form-card">
            <h2><i class="fas fa-user-shield"></i> Gestion Face ID</h2>
            <p style="color:#777;">Suivez les utilisateurs ayant configurÃ© une connexion par visage.</p>
            <p><a href="index.php?action=admin-faces" class="btn-primary"><i class="fas fa-list"></i> Ouvrir la gestion Face ID</a></p>
            <div class="table-container">
                <table>
                    <thead><tr><th>Utilisateur</th><th>Email</th><th>Statut</th><th>Mis Ã  jour</th><th>Actions</th></tr></thead>
                    <tbody>
                    <?php foreach ($users as $u): ?>
                        <tr>
                            ...
                            <td>
                                <a href="index.php?action=admin-view-face&user_id=<?= (int)$u['id_utilisateur'] ?>" class="btn-outline btn-sm"><i class="fas fa-eye"></i> Voir</a>
                                <?php if (!empty($u['has_face'])): ?>
                                    <form method="post" action="index.php?action=admin-delete-face&user_id=<?= (int)$u['id_utilisateur'] ?>" style="display:inline;" onsubmit="return confirm('Supprimer ce Face ID ?')">
                                        <button type="submit" class="btn-outline btn-sm"><i class="fas fa-trash"></i> Supprimer</button>
                                    </form>
                                <?php endif; ?>
                            </td>
                        </tr>
...
<script>
    ...
    function sortUsers(type) {
        ...
        rows.sort((a, b) => {
            if (type === 'alpha') {
                const nomA = a.cells[0].textContent.trim().toLowerCase();
                const nomB = b.cells[0].textContent.trim().toLowerCase();
