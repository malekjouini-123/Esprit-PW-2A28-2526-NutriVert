...
$completionTitle   = $completionTitle   ?? 'Coaching Terminé!';
$completionMessage = $completionMessage ?? 'Vous avez complété avec succès tous les exercices de ce programme. Excellent travail!';
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .feedback-card {
            ...
            text-align: left;
            background: #f8fbf9;
            border: 1px solid rgba(74, 107, 74, 0.15);
            border-radius: 14px;
            padding: 18px;
        }
        ...
        .emoji-option:has(input:checked) {
            ...
            box-shadow: 0 0 0 2px rgba(125, 207, 182, 0.25);
        }
        ...
        .btn-feedback:hover { background: #e56a55; }
...
<div style="padding-top: 80px;">
    <div class="completion-card">
        ...
        <h1 class="completion-title"><?= htmlspecialchars($completionTitle) ?></h1>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-item">
                ...
                <div class="stat-value">
                    <?= htmlspecialchars((string)$completedCount) ?> / <?= htmlspecialchars((string)$totalCount) ?>
                </div>
            </div>
        </div>
        ...
        <?php if (!empty($showFeedbackForm) && !empty($coachingIdForFeedback)): ?>
            <div class="feedback-card">
                <div class="feedback-title">💬 Votre avis sur ce coaching :</div>
                <p style="margin: 0 0 12px 0; color: #556; font-size: 0.92rem;">
                    Choisissez un emoji et laissez un commentaire.
                </p>
                <form method="post" action="index.php?controller=user_dashboard&action=feedback">
                    <input type="hidden" name="coaching_id" value="<?= (int)$coachingIdForFeedback ?>">

                    <div class="emoji-grid" role="radiogroup" aria-label="Choisir un emoji">
                        <?php foreach (['👍', '🔥', '😐', '❤️', '😴'] as $idx => $em): ?>
                            <label class="emoji-option" title="<?= htmlspecialchars($em) ?>">
                    ...
                    <textarea
                        ...
                        maxlength="1000"
                        placeholder="Écris un commentaire court sur ce coaching..."
                        required
                    ></textarea>
        ...
        <!-- Actions (shown only when no feedback form) -->
        <?php if (empty($showFeedbackForm)): ?>
            <div class="actions">
...
<script>
    const startedAt = <?= intval($startedAt ?? time()) ?>;
