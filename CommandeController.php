...
class CommandeController
{
        ...
        try {
            switch ($method) {
                ...
                case 'PUT':
                    ...
                    $this->updateCommande($id, $parsed['nom'], $parsed['client_email'], $parsed['statut']);
                    nv_json(['ok' => true]);
    ...
    {
        $sql = 'SELECT c.id, c.nom, c.date_commande, c.statut, c.client_email, c.total,
    ...
    private function findByIdWithLignes(int $id): ?array
    {
        $stmt = $this->pdo->prepare(
            'SELECT id, nom, date_commande, statut, client_email, total FROM commande WHERE id = ?'
        );
        $stmt->execute([$id]);
        $row = $stmt->fetch();
        if ($row === false) {
    ...
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO commande (nom, date_commande, statut, client_email, total) VALUES (?, NOW(), ?, ?, 0)'
        );
        $stmt->execute([$commande->getNom(), $commande->getStatut(), $commande->getClientEmail()]);
        return (int) $this->pdo->lastInsertId();
    }

    private function updateCommande(int $id, string $nom, ?string $clientEmail, string $statut): int
    {
        $stmt = $this->pdo->prepare(
            'UPDATE commande SET nom = ?, statut = ?, client_email = ? WHERE id = ?'
        );
        $stmt->execute([$nom, $statut, $clientEmail, $id]);
    ...
    private function addOrUpdateLigne(int $commandeId, int $produitId, int $quantite): void
    {
        ...
        $this->pdo->beginTransaction();
        try {
            $pl = $this->pdo->prepare('SELECT prix, combien, nom FROM produit WHERE id = ? FOR UPDATE');
    ...
    /**
     * @param array<string,mixed> $in
     * @return array{nom:string,client_email:?string,statut:string}|null
     */
