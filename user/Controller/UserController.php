...
class UserController
{
    ...
    public function handle(string $action): void
    {
        switch ($action) {
            ...
            case 'addFace':           $this->addFace();            break;
            case 'deleteFace':        $this->deleteFace();         break;
    ...
    private function register(): void
    {
        ...
        try {
            ...
            $faceImage = trim((string)($_POST['face_image'] ?? ''));
            if ($faceImage !== '' && $newUserId > 0) {
                try {
                    $faceResult = $this->faceRecognition->encodeCapturedImage($faceImage, $newUserId);
                    if ($faceResult['success'] && !empty($faceResult['data']['encoding'])) {
                        $encoding = is_string($faceResult['data']['encoding'])
                            ? $faceResult['data']['encoding']
                            : json_encode($faceResult['data']['encoding']);
                        $faceStmt = $this->pdo->prepare(
                            'INSERT INTO visages_utilisateurs (id_utilisateur, face_encoding) VALUES (:uid, :enc)
                             ON DUPLICATE KEY UPDATE face_encoding = :enc2'
                        );
                        $faceStmt->execute([':uid' => $newUserId, ':enc' => $encoding, ':enc2' => $encoding]);
                    }
                } catch (\Exception $e) {
                    error_log('Face ID enroll error: ' . $e->getMessage());
                }
            }
    ...
    private function forgotPassword(): void
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($token) {
                ...
                include __DIR__ . '/../../auth/View/email-confirmation.php';
                exit;
            } else {
        ...
        include __DIR__ . '/../../auth/View/forgot-password.php';
        exit;
    }
    ...
    private function resetPassword(): void
    {
        ...
        $flashMessage = $_SESSION['flash_message'] ?? null;
        unset($_SESSION['flash_message']);
        include __DIR__ . '/../../auth/View/reset-password.php';
        exit;
    }
    ...
    private function adminUsers(): void
    {
        ...
        $flashMessage = $_SESSION['flash_message'] ?? null;
        unset($_SESSION['flash_message']);
        include __DIR__ . '/../../admin/View/users.php';
        exit;
    }

    // =========================================================================
    //  FACE ID — GESTION DEPUIS LE DASHBOARD UTILISATEUR
    // =========================================================================
    ...
    private function addFace(): void
    {
        ...
        $faceImage  = trim((string)($payload['face_image'] ?? ''));
        ...
        $result = $this->faceRecognition->encodeCapturedImage($faceImage, $userId);

        if (!$result['success'] || empty($result['data']['encoding'])) {
            $this->jsonResponse([
                'success' => false,
                'error'   => $result['data']['error'] ?? 'Impossible d\'encoder le visage.',
            ], 422);
        }

        $encoding = is_string($result['data']['encoding'])
            ? $result['data']['encoding']
            : json_encode($result['data']['encoding']);

        $stmt = $this->pdo->prepare(
            'INSERT INTO visages_utilisateurs (id_utilisateur, face_encoding) VALUES (:uid, :enc)
             ON DUPLICATE KEY UPDATE face_encoding = :enc2'
        );
        $stmt->execute([':uid' => $userId, ':enc' => $encoding, ':enc2' => $encoding]);

        $this->jsonResponse(['success' => true, 'message' => 'Face ID enregistré avec succès.']);
    }

    private function deleteFace(): void
    {
        if (empty($_SESSION['user'])) {
            $this->redirect('index.php?view=login');
        }
