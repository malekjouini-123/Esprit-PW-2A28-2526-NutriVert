...
class EmailService {
    ...
    private function checkPHPMailer(): void {
        // When Google OAuth credentials are configured, use raw SMTP with XOAUTH2 instead of PHPMailer AUTH LOGIN.
        if (!empty($this->config['google']['client_id'])
            && !empty($this->config['google']['client_secret'])
            && !empty($this->config['google']['refresh_token'])) {
            return;
        }
    ...
    private function initializeWithPHPMailer(): void {
        ...
        try {
            ...
            $this->mailer->SMTPOptions = ['ssl' => ['verify_peer' => false, 'verify_peer_name' => false]];
